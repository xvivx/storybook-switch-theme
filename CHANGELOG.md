# v10.0.10 (Wed Jun 10 2026)

#### ⚠️ Pushed to `main`

- feat: update doc ([@xvivx](https://github.com/xvivx))
- feat: remove pr workflow ([@xvivx](https://github.com/xvivx))

#### Authors: 1

- xvivx ([@xvivx](https://github.com/xvivx))

---

# v10.0.9 (Fri Jun 05 2026)

#### 🐛 Bug Fix

- Update workflow [#2](https://github.com/xvivx/storybook-switch-theme/pull/2) ([@xvivx](https://github.com/xvivx))

#### Authors: 1

- xvivx ([@xvivx](https://github.com/xvivx))

---

# v10.0.8 (Thu Jun 04 2026)

#### ⚠️ Pushed to `main`

- fix: addon id ([@aaaabc](https://github.com/aaaabc))

#### Authors: 1

- [@aaaabc](https://github.com/aaaabc)

---

# v10.0.7 (Thu Jun 04 2026)

#### ⚠️ Pushed to `main`

- fix: addon id ([@aaaabc](https://github.com/aaaabc))

#### Authors: 1

- [@aaaabc](https://github.com/aaaabc)

---

# v10.0.6 (Thu Jun 04 2026)

#### ⚠️ Pushed to `main`

- fix: definePreview bug ([@aaaabc](https://github.com/aaaabc))

#### Authors: 1

- [@aaaabc](https://github.com/aaaabc)

---

# v10.0.5 (Thu Jun 04 2026)

#### ⚠️ Pushed to `main`

- fix: packagejson ([@aaaabc](https://github.com/aaaabc))

#### Authors: 1

- [@aaaabc](https://github.com/aaaabc)

---

# v10.0.4 (Thu Jun 04 2026)

#### 🐛 Bug Fix

- Complete storybook-addon-theme [#1](https://github.com/xvivx/storybook-switch-theme/pull/1) ([@xvivx](https://github.com/xvivx))

#### ⚠️ Pushed to `main`

- feat: update workflows ([@aaaabc](https://github.com/aaaabc))
- Initial commit ([@xvivx](https://github.com/xvivx))

#### Authors: 2

- [@aaaabc](https://github.com/aaaabc)
- xvivx ([@xvivx](https://github.com/xvivx))
